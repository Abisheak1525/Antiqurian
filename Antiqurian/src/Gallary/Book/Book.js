...
const books = [
  ...
  // You mentioned status updates (Available, Reserved, Sold) — how did you design the database schema to handle these states efficiently?

  // In my project, each book had a status field to indicate whether it was Available, Reserved, or Sold.
  // I handled this at the database level by keeping a status column in the books table (using ENUM or integer codes for efficiency),
  // and at the application layer I enforced valid transitions, so you couldn’t move from Sold → Reserved. On the frontend, I displayed 
  // the status with color codes and even filtered out unavailable books in search results. This made it easy for users to see real-time 
  // availability while keeping the backend clean and consistent
